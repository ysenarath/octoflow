...
class ExperimentReader:
    ...
    def read_metadata(self, run_path: Path) -> dict:
        raise NotImplementedError
...
def import_to_db(reader: ExperimentReader) -> None:
    for run_path in tqdm.tqdm(
        ...
        expr_meta = reader.read_experiment_meta(run_path)
...
class MLflowExperimentReader(ExperimentReader):
    def count_runs(self) -> int:
        return len(list(self.expr_dir.glob("mlruns/0/runs/*")))

    def iter_runs(self) -> Generator[RunMeta, None, None]:
        for run_path in self.expr_dir.glob("mlruns/0/runs/*"):
            if not run_path.is_dir():
                continue
    ...
    def read_metadata(self, run_path: Path) -> dict:
        return {"creation_time": run_path.stat().st_ctime}
